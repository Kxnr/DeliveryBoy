...
public class DeliveryItems : MonoBehaviour
{
    ...
    public AudioClip PopItem(string storeName)
    {
        ...
        {
            int currentSessionNumber = UnityEPL.GetSessionNumber();
            System.Random rng = new System.Random(currentSessionNumber);
            randomItemIndex = rng.Next(remainingItems.Length);
            randomItemName = remainingItems[randomItemIndex];
        }
        else
        {
            // Get the item
            randomItemIndex = Random.Range(0, remainingItems.Length);
            randomItemName = remainingItems[randomItemIndex];
        }
    ...
    public static bool ItemsExhausted()
    {
            ...
            foreach (string remainingItemsPath in remainingItemsPaths)
            {
                ...
                bool storeExhausted;
                if (UnityEPL.GetSessionNumber() == 1)
                {
                    storeExhausted = itemsRemaining.Length <= 71; // Running the game for 7 delivery days
                }
                else
                {
                    storeExhausted = itemsRemaining.Length <= 26; // Running the game for 10 delivery days
                }
    ...
    public System.Random ReliableRandom()
    {
        ...
        {
            participantCode = "0";
            codeParts = participantCode.Split('_');
            return new System.Random(codeParts[0].GetHashCode());
        }
        else
        {
            participantCode = UnityEPL.GetParticipants()[0];
            codeParts = participantCode.Split('_');
            return new System.Random(codeParts[0].GetHashCode());
        }
    }
